...
class FasterRCNNBase(nn.Module):
    ...
    def forward(self, images, targets=None):
        ...
        images, targets = self.transform(images, targets)  # 对图像进行预处理 对数据标准化处理 resize到最小 最大范围

        # print(images.tensors.shape)
        features = self.backbone(images.tensors)  # 将图像输入backbone得到特征图
        if isinstance(features, torch.Tensor):  # 若只在一层特征层上预测，将feature放入有序字典中，并编号为‘0’
            features = OrderedDict([('0', features)])  # 若在多层特征层上预测，传入的就是一个有序字典 用于FPN+backbone中
...
class FasterRCNN(FasterRCNNBase):
    ...
    def __init__(self, backbone, num_classes=None,
                 ...
                 rpn_post_nms_top_n_train=2000, rpn_post_nms_top_n_test=1000,  # rpn中在nms处理后保留的proposal数 这里主要是针对有FPN的网络 每层FPN都包含2000个proposal
